...
只使用以下状态词，避免中英混写或自由描述：

| 状态 | 含义 | 主要写入者 |
|---|---|---|
| `设计中` | 需求、边界、验收或验证计划仍在整理。 | `spec` |
| `待实施` | SPEC 已写清目标、边界、验收和验证计划，可以进入 `plan` 或 `dev`。 | `spec` |
| `开发中` | 已开始按 SPEC / PLAN 修改代码或文档。 | `dev` |
| `验证中` | 改动完成但新鲜验证尚未通过，或验证失败后等待修复。 | `dev` |
| `验证通过` | 本轮新鲜验证通过，等待最终 `ship`。 | `dev` |
| `已完成` | 最终验证、SPEC / INDEX 更新和交付动作已完成。 | `ship` |
| `阻塞` | 缺少必要信息、外部依赖不可用或连续验证无法推进。 | 任一流程 |

标准流转为：`设计中 -> 待实施 -> 开发中 -> 验证中 -> 验证通过 -> 已完成`。`阻塞` 可以从任意状态进入；解除阻塞后回到被阻塞前的下一步。
