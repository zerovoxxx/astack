...
1. **新项目 bootstrap 有鸡生蛋的门槛**。`astack subscribe <skill>` 之前，项目得先有 `.claude/` 目录 + 治理文档骨架（`AGENTS.md`、`docs/astack/INDEX.md` 等）。用户当前只能去终端运行独立脚本，操作链条长。
...
**不再需要：** 解析 `AGENTS.md + docs/astack/INDEX.md` 的存在性 —— 这是个跟 harness skill 安装状态无关的正交信号（项目可以不用 harness 命令体系但保留 `AGENTS.md`，反之亦然）。
  ...
  export const SYSTEM_SKILLS: Array<{ id: string; name: string; description: string }> = [
    {
      ...
      description: "初始化或迁移项目的 Harness 研发流程治理基础设施（CLAUDE.md + AGENTS.md + docs/astack/），为 spec/plan/dev/ship skill 体系打底。"
    }
  ];
  ```
...
This creates CLAUDE.md, AGENTS.md, and docs/astack/ scaffolding.
