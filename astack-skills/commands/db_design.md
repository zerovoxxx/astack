---
description: "🗄️ 数据库设计 - 根据业务需求生成符合 FiT 规范的 MySQL 库表设计文档与 DDL"
---

根据 $ARGUMENTS 进行数据库设计，输出符合 FiT 数据库设计规范的库表设计文档与 DDL 语句。

若用户未指定需求文档，提示用户提供业务需求描述或指定 Spec 文件路径。
若用户指定了迭代编号（如 `9`），自动匹配对应的 `Iteration{N}_*.md` 文件中的数据模型章节。

## 📊 当前状态

### 项目架构信息
!`head -20 AGENTS.md 2>/dev/null | grep -E '(项目名|技术基线|基础包名|当前架构)'`

### 已有 domain 子领域
!`ls -d finclaw-platform-ao/finclaw-platform-ao-server/src/main/java/com/tencent/fit/finclaw/platform/ao/domain/*/ 2>/dev/null | xargs -I{} basename {}`

### 已有数据库设计文档
!`ls -1 docs/db/ 2>/dev/null || echo "(暂无 docs/db/ 目录)"`

## 上下文准备

设计前必须获取：
- `AGENTS.md`：项目宪法、业务子领域定义、分层规范
...
2. 确认涉及的 domain 子领域（必须属于 AGENTS.md 中定义的 7 个子领域之一）
...
2. **领域归属**：表必须归属于 AGENTS.md 定义的 7 个业务子领域之一，不允许创建游离于领域之外的表。
3. **不执行 DDL**：本命令只输出设计文档和 DDL 语句，不在数据库中执行任何操作。
...
6. **反范式有度**：为满足单表查询要求可适当反范式，但必须说明冗余字段的同步策略。
7. **禁止预留字段**：禁止 Fstandby 等无明确业务含义的预留字段，所有字段必须有明确的业务语义。需要扩展时通过 ALTER TABLE 新增字段。（注：iWiki 建议单库单表/百库十表预留 standby 字段，本项目采用更严格策略以保证字段语义清晰）
8. **逻辑删除**：生产数据只允许逻辑删除，严禁物理 DELETE。所有表必须包含 `Flstate`（tinyint, DEFAULT 1），1 = 正常、2 = 已删除。业务查询默认过滤 `Flstate = 1`。
