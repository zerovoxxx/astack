...
（待补充：项目专属技术栈、架构边界、命名规则、领域红线或发布流程。）

### 3.1 项目质量门

> `spec`、`plan`、`dev`、`ship` 从本节读取项目真实验证入口，不猜测其他技术生态的默认命令。优先复用仓库已有 wrapper 或 package script；不存在的门标记为“不适用”。

| 质量门 | 项目命令 |
|---|---|
| 文档 / skill | （待补充：whitespace 与针对性过期引用检查） |
| 静态检查 / 编译 / 类型检查 | （待补充或标记不适用） |
| 聚焦测试 | （待补充：可替换为当前模块和测试目标的仓库命令） |
| 完整测试 | （待补充或标记不适用） |
| 最终构建 / 验证 | （待补充：非修改型命令） |
